workspace {
    model {
        ...
        bexioMcp = softwareSystem "bexio-mcp" "MCP server exposing bexio timesheet and lookup operations as tools" {
            mcpServer = container "MCP Server" "Handles MCP protocol, tool registration, handlers for timesheet tools (create/search/edit/delete) and lookup tools, and resolves default user_id/status_id from API lookups. Single search_timesheets tool dispatches to GET or POST based on search_fields presence." "Go (go-sdk/mcp)"
